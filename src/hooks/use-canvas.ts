...
interface DraftShape {
  type: "frame" | "arrow" | "line" | "rect" | "ellipse";
  startWorld: Point;
  currentWorld: Point;
}
...
export const useInfinteCanvas = () => {
  ...
  const touchMapRef = useRef<Point[]>([]);
  ...
  const freehandTrick = (): void => {
    ...
    if (isDrawingRef.current) {
      freehandRafRef.current = window.requestAnimationFrame(freehandTrick);
    }
  };
  ...
  const onPointerDown: React.PointerEventHandler<HTMLDivElement> = (e) => {
    ...
    if (touchMapRef.current.length <= 1) {
      ...
      if (e.button === 0) {
        if (currentTool === "select") {
          ...
          if (
            ...
            freehandRafRef.current =
              window.requestAnimationFrame(freehandTrick);
            requestRender();
          }
        }
      }
          ...
          if (
            ...
            if (
              ...
              dispatch(
                updateShape({
                  id,
                  patch: { x: initialPos.x + deltaX, y: initialPos.y + deltaY },
                }),
              );
            }
          } else if (shape.type === "freedraw") {
  ...
  const finalizeDrawingIfAny = (): void => {
    ...
    if (draft) {
      ...
      if (w > 1 && h > 1) {
        if (draft.type === "frame") {
          console.log("creating frame", { x, y, w, h });
  ...
  useEffect(() => {
    const handleResizeStart = (e: CustomEvent) => {
      ...
      resizeDataRef.current = {
        ...
        startPoint: { x: e.detail.clientX, y: e.detail.clientY || 0 },
      };
    };
